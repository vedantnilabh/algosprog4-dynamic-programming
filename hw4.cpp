...
pair<vector<vector<float>>, vector<vector<int>>> WWWWW_2(vector<float> w, vector<float> p, int s, int t) {
    ...
    for (int i = j - 2; i >= 0; i--) {
        ...
        float lifeline_case2 = dp[0][i+1] * 1.0f + w[min] - (i * 10);
        ...
        float lifeline_case3_get_through = dp[2][i+1] * 1.0f + w[min] - (i * 10);
        dp[3][i] = fmax(fmax(fmax(no_lifeline_case3, lifeline_case3_get_easier), lifeline_case3_get_through), w[i]);

        quit_decision[0][i] = (dp[0][i] == w[i]) ? 1 : 0;
        quit_decision[1][i] = (dp[1][i] == w[i]) ? 0 : (dp[1][i] == no_lifeline_case1) ? 1 : 2;        quit_decision[2][i] = (dp[2][i] == w[i]) ? 0 : (dp[2][i] == no_lifeline_case2) ? 1 : 2;
