...
pair<vector<vector<float>>, vector<vector<int>>> WWWWW_1(vector<float> w, vector<float> p, int s, int t) {
    ...
    for (int i = j - 2; i >= 0; i--) {
        ...
        float no_lifeline = dp[0][i+1] * p[i+1] + (1 - p[i+1]) * w[min];
        float lifeline = dp[1][i+1] * p_get_easier + (1 - p_get_easier) * w[min];
...
pair<vector<vector<float>>, vector<vector<int>>> WWWWW_2(vector<float> w, vector<float> p, int s, int t) {
    ...
    for (int i = j - 2; i >= 0; i--) {
        ...
        float lifeline_case1 = dp[2][i+1] * p_get_easier + (1 - p_get_easier) * w[min];
        ...
        float lifeline_case2 = dp[3][i+1] * 1.0f + w[min] - (i + 1) * 10;
        ...
        float lifeline_case3_get_easier = dp[3][i+1] * p_get_easier + (1 - p_get_easier) * w[min];
        float lifeline_case3_get_through = dp[3][i+1] * 1.0f + w[min] - (i + 1) * 10;
